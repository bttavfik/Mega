    ...
    public partial class frmReturnToSupplier : Form
    {
        ...
        private void frmReturnToSupplier_Load(object sender, EventArgs e)
        {
            ...
            if (Edit_Flage)
            {
                ...
                var returnToSupplier = mega.ReturnToSuppliers.Where(x => x.Id == this.Id).FirstOrDefault();

                dtpIssuedDate.Value = returnToSupplier.ReturnDate;
                txtRefer1.Text = returnToSupplier.Reference;
                cboApplicant.SelectedValue = returnToSupplier.ApplicantCode;
                cboApprover.SelectedValue = returnToSupplier.ApproverCode;
                cboProject.SelectedValue = returnToSupplier.ProjectId;
                txtPurpose.Text = returnToSupplier.Remark;

                var returnToSupplierView = mega.ReturnToSupplierDetails.Where(x => x.MasterCode == this.Id).ToList();
